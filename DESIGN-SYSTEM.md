...
| **Product surfaces** | Full-screen or flow demos tied to an app area (Home, Fields, Weather, Scout, Trials, Yield, …). | [`pages/<area>/`](./pages/) |
| **Figma / MCP capture** | Push live pages into Figma via MCP or html.to.design. | Serve **`index.html`** or **`pages/<area>/…`** over HTTP — see [`docs/figma-mcp.md`](./docs/figma-mcp.md) |

**Authoring rule:** UI that maps to a primary product destination should live under the matching `pages/<area>/` folder (not loose in `pages/` root). Bulk export-only HTML is not kept in a separate repo folder by default. Extend tokens and `components/*.css` before inventing parallel styles in a demo.
...
| `ai-assistant-widget.css` | Floating LLM assistant: type or mic (STT); examples in `index.html` (AI assistant widget section) |
